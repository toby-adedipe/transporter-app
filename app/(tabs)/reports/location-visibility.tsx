...
import { useGetLocationVisibilityQuery } from '@/store/api/visibilityApi';
...
export default function LocationVisibilityScreen() {
  ...
  const { data, isLoading, isFetching, refetch } = useGetLocationVisibilityQuery(
    filter,
  ...
  const renderItem = ({ item }: { item: any }) => (
    <Card variant="default" padding="base">
      <View style={styles.row}>
        <Text style={styles.name}>{item.locationCategory ?? item.location ?? 'N/A'}</Text>
      ...
      <Text style={styles.detail}>Trucks: {item.truckCount ?? item.totalTrucks ?? 'N/A'}</Text>
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>Location Visibility</Text>
        <View style={{ width: 32 }} />
      </View>
      <VisibilityFilters />
      {isLoading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<EmptyState icon="location-outline" title="No location data" subtitle="Try adjusting your filters" />}
        />
      )}
    </View>
  );
