...
import { useGetNoGoZoneVisibilityQuery } from '@/store/api/visibilityApi';
import { colors, spacing, fontSize, fontWeight } from '@/constants/theme';

export default function NoGoZoneScreen() {
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const filter = useVisibilityFilter();
  const { data, isLoading, isFetching, refetch } = useGetNoGoZoneVisibilityQuery(
    filter,
  ...
  const renderItem = ({ item }: { item: any }) => (
    <Card variant="default" padding="base">
      <View style={styles.row}>
        <Text style={styles.name}>{item.zoneName ?? item.geofenceName ?? item.location ?? 'N/A'}</Text>
        <StatusBadge label="Alert" status="danger" />
      </View>
      <Text style={styles.detail}>Truck: {item.truckPlate ?? item.registrationNumber ?? 'N/A'}</Text>
      <Text style={styles.detail}>Status: {formatStatus(item.status ?? item.truckStatus)}</Text>
      {item.enteredAt && <Text style={styles.detail}>Entered: {item.enteredAt}</Text>}
    </Card>
  );
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      <View style={styles.header}>
        ...
        <Text style={styles.title}>No-Go Zones</Text>
        <View style={{ width: 32 }} />
      </View>
      <VisibilityFilters />
      {isLoading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<EmptyState icon="warning-outline" title="No violations" subtitle="No trucks in restricted zones" />}
        />
      )}
    </View>
  );
